...
class InsuranceCompany implements Cloneable{
    ...
    // Method to create a shallow copy of users
    ...
    // Method to sort all users in the company based on city name
    public void sortUsersByCity() {
        Collections.sort(users);
    }

    // Method to sort all users in the company based on total premium payments
    public void sortUsersByTotalPremiums(int flatRate) {
        //Todo
    }

    // Clone method to support deep cloning of the company
    //There might be something that is going wrong in this method
    @Override
    public InsuranceCompany clone() throws CloneNotSupportedException {

        InsuranceCompany clonedCompany = (InsuranceCompany) super.clone();
        clonedCompany.users = deepCopyUsers();
